...
# /usr/bin/gpg --batch --no-tty --encrypt --cipher-algo AES256 --compress-algo none -r 1539150C

# aws:
# echo "hello world" | aws s3 cp - s3://some-bucket/hello.txt
# ^^^^
#if DEEPARCHIVE:
#aws_string = '/usr/local/bin/aws s3 cp --profile deeparchive --storage-class DEEP_ARCHIVE '+encrypted_file_path+' s3://s3doj/'+file_year+'/'+file_month+'/'
#else:
